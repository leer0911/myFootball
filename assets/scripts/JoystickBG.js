...
cc.Class({
  ...
  _getAngle: function(point) {
    ...
    this._angle =
      Math.atan2(point.y - pos.y, point.x - pos.x) * (180 / Math.PI);
    return this._angle;
  },
  ...
  _touchStartEvent: function(event) {
    const animation = this._playerNode.getComponent(cc.Animation);
    animation.play('run');
  ...
  _touchMoveEvent: function(event) {
    ...
    if (radius > distance) {
      ...
      var x =
        ...
        Math.cos(this._getRadian(cc.p(posX, posY))) * radius;
      var y =
        this.node.getPosition().y +
        Math.sin(this._getRadian(cc.p(posX, posY))) * radius;
